...
async function getRecipes() {
  ...
  const _recipes=localStorage.getItem('recipes');
  if(_recipes) return JSON.parse(_recipes); 
  /**************************/
  ...
  try{
    ...
    for(const URLs of RECIPE_URLS){
      var response=await fetch(URLs);
      const re=await response.json();
      hold_recipes.push(re);

    }
    saveRecipesToStorage(hold_recipes);
    resolve(hold_recipes);
